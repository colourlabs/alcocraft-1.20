...
public class DiggerBitterMugBlock extends MugBlock {
    public DiggerBitterMugBlock(Properties properties) { super(properties); }

    @Override
    public void animateTick(BlockState pState, Level pLevel, BlockPos pPos, RandomSource pRand) {
        if (pRand.nextFloat() < 0.4f) {
            double d0 = pPos.getX() + 0.4D + pRand.nextDouble() * 0.2D;
            double d1 = pPos.getY() + 0.2D + pRand.nextDouble() * 0.18D;
            double d2 = pPos.getZ() + 0.4D + pRand.nextDouble() * 0.2D;
